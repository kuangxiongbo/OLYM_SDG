...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.wizard').forEach(wizard => {
    ...
    const contentContainer = wizard.nextElementSibling || wizard.parentElement.querySelector('.wizard-content');
      ...
      if (navNext) {
        navNext.addEventListener('click', () => {
          ...
          if (currentIndex < steps.length - 1) {
            ...
            // 最后一步，完成操作
            if (typeof onWizardComplete === 'function') {
              onWizardComplete();
            }
          }
        });
  ...
  document.querySelectorAll('.tab-menu').forEach(menu => {
    ...
    const container = menu.nextElementSibling || menu.parentElement.querySelector('.tab-content');
    if (!container) {
      return;
    }
    ...
    const initialTab = menu.querySelector('button[data-tab-target].active') || buttons[0];
    if (initialTab) {
